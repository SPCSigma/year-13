...
def get_table_data(table_name, search_columns, selected_columns, search_data, sort_column, sort_type):
    ...
    if 'card_price' in columns_to_select:
        columns_to_select = columns_to_select.replace('card_price', "printf('%.2f', card_price) AS card_price")
    if 'total' in columns_to_select:
        columns_to_select = columns_to_select.replace('total', "printf('%.2f', total) AS total")

    # Build the WHERE clause dynamically from the list of searchable columns
    where_sql = " OR ".join([f"{col} LIKE ?" for col in search_columns])
    
    sql = f"""
    SELECT {columns_to_select} FROM {table_name}
    WHERE ({where_sql})
    ORDER BY {sort_column} {sort_type}
    """

    search_query = f'%{search_data}%'
    search_parameters = tuple([search_query] * len(search_columns))

    # logging.debug(f"Running SQL code {sql} with parameters {search_parameters}")
    table_items = c.execute(sql, search_parameters).fetchall()
    conn.close()
    return table_items
...
def index():
    table_name = 'tbl_cards'
    search_columns = ['card_id', 'card_name', 'card_rarity', 'card_price']
    # Default values
    ...
    data = {}
    data = get_table_data(table_name, search_columns, selected_columns, search_data, sort_column, sort_type)
...
def edit_card():
    ...
    WHERE card_id = ?
    """
    affected_rows = c.execute(sql, update_card_data).rowcount
...
def add_card():
    ...
    sql = """INSERT INTO tbl_cards (card_name, card_rarity, card_price)
    VALUES (?, ?, ?)
    """
...
def purchases():
    ...
    table_name = 'tbl_purchases'
    search_columns = ['purchase_id', 'purchase_date', 'name_of_purchaser', 'total', 'delivery_address', 'email_address']
    conn = get_db_connection()
    c = conn.cursor()
    ...
    # Admin session
    admin = session.get('admin', False)
    return render_template("purchases.html", purchase=data, search_data=search_data, selected_columns=selected_columns, 
                           sort_type=sort_type, sort_column=sort_column, username=username, admin=admin, flash_category=flash_category)
